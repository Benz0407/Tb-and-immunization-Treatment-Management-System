...
import 'package:management_of_immunizataion_and_tuberculosis_treatment/screens/tb_details_screen.dart';
...
class _TbScreenState extends State<TbScreen> {
  late Future<List<Tuberculosis>> futureTuberculosis;
  ...
  void refreshTbList() {
    setState(() {
      futureTuberculosis = TbService().fetchTuberculosis();
      _checkForEvents();
    });
  }
  ...
  void searchTuberculosis(String query) {
    setState(() {
      _searchQuery = query;
      futureTuberculosis = TbService().searchTbRecords(query);
    });
  }
            ...
            child: FutureBuilder<List<Tuberculosis>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  List<Tuberculosis> tbRecords = snapshot.data!;
                  List<Tuberculosis> filteredList = _searchQuery.isEmpty
                      ? tbRecords
                      : tbRecords
                          .where((tb) =>
                              tb.id
